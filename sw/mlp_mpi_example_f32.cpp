...
typedef shared_buffer::ptr_t ptr_t;

#define DETAILED_PROFILE
#define N_PROF_THREADS 128

#define IKA_CTRL_ADDR              0x030
#define IKA_WR_DATA                0x038
#define IKA_RD_DATA                0x040
#define NUM_NODES                  3


typedef struct fpga_rqst {
  int idx;
  int count;
  float* buf;
  volatile float* fpga_buf;
} fpga_rqst;

int done_idx;

int ikl_setup() {
    int system_cmd_result;
    char commandline[ 256 ];
    //reset ikl
    snprintf( commandline, sizeof (commandline), "iko areset");
    system_cmd_result = system(commandline);
    snprintf( commandline, sizeof (commandline), "iko reset");
    system_cmd_result = system(commandline);
...
int all_reduce_setup(volatile uint32_t* done, int count, int node_id, OPAE_SVC_WRAPPER &fpga) {
    int total_length = count >> 4; //each message is 16 uint32_t numbers
    //setup number of nodes and node ID
    uint64_t node_info = (NUM_NODES << 16) + node_id;
    fpga.write_csr64(IKA_WR_DATA, node_info);
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0x8>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x8>>2)));
    uint64_t reg_data = fpga.read_csr64(IKA_RD_DATA);
    printf("node_info written: %lx\n", reg_data);

    //setup table total size
    fpga.write_csr64(IKA_WR_DATA, total_length);
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0xC>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0xC>>2)));
    reg_data = fpga.read_csr64(IKA_RD_DATA);
    printf("total_length written: %lx\n", reg_data);

    //setup done buffer address
    printf("done addr: %lx\n", intptr_t(done));
    uint64_t tmp = intptr_t(done) >> 6;
    fpga.write_csr64(IKA_WR_DATA, tmp);
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0x1C>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x1C>>2)));
    reg_data = fpga.read_csr64(IKA_RD_DATA);
    printf("done_addr_l: %lx\n", reg_data);

    fpga.write_csr64(IKA_WR_DATA, (tmp>>32));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0x20>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x20>>2)));
    reg_data = fpga.read_csr64(IKA_RD_DATA);
    printf("done_addr_h: %lx\n", reg_data);

    return 0;
}

uint64_t get_all_reduce_latency(OPAE_SVC_WRAPPER &fpga) {
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x14>>2)));
    uint64_t reg_data = fpga.read_csr64(IKA_RD_DATA);
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x10>>2)));
    reg_data = (reg_data << 32) + fpga.read_csr64(IKA_RD_DATA);
    return reg_data;
}

uint64_t get_host_stall_cycles(OPAE_SVC_WRAPPER &fpga) {
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x4C>>2)));
    uint64_t reg_data = fpga.read_csr64(IKA_RD_DATA);
    return reg_data;
}
...
int all_reduce(volatile float* buf, volatile uint32_t* done, int count, OPAE_SVC_WRAPPER &fpga, fpga_rqst *rqst_id) {
    ...
    // send the buffer pointer to FPGA
    uint64_t tmp = intptr_t(buf) >> 6;
    // printf("mem addr: %lx\n", intptr_t(buf));
    fpga.write_csr64(IKA_WR_DATA, tmp);
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0x40>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x40>>2)));
    // uint64_t reg_data = fpga.read_csr64(IKA_RD_DATA);
    // printf("mem_addr_l: %lx\n", reg_data);

    fpga.write_csr64(IKA_WR_DATA, (tmp>>32));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x13000 | (0x44>>2)));
    fpga.write_csr64(IKA_CTRL_ADDR, (0x23000 | (0x44>>2)));
    // reg_data = fpga.read_csr64(IKA_RD_DATA);
...
int wait(volatile uint32_t* done_buf, fpga_rqst *rqst_id) {
    // struct timeval w_start, w_end;

    // gettimeofday(&w_start, 0);
    // clock_t c_start = clock();

    uint32_t done = done_buf[(rqst_id->idx) << 4];
    while (0 == done)
    {
        //nanosleep(&pause, NULL);
        done = done_buf[(rqst_id->idx) << 4];
    };

    // gettimeofday(&w_end, 0);
    // clock_t c_end = clock();

    // long w_sec = w_end.tv_sec - w_start.tv_sec;
    // long w_usec = w_end.tv_usec - w_start.tv_usec;
    // double wall_time = w_sec*1000.0 + w_usec*1e-3;
...
int main(int argc, char* argv[])
{
  ...
  auto done_buf_handle = fpga.allocBuffer(4096);
  auto done_buf = reinterpret_cast<volatile uint32_t*>(done_buf_handle->c_type());
  assert(NULL != done_buf);

  fpga_rqst request[2];
  all_reduce_setup(done_buf, C[0]*C[0], rank, fpga);
